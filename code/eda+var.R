...
setwd("/Users/apple/Downloads/411_ASSIGMENT2/411_lab7")

df_poultry <- read.csv("sample1_sales_timeseries.csv", header = TRUE, 
                       colClasses = c(date="Date", store_nbr = "numeric", sales = "numeric", onpromotion = "numeric"))
...
for (i in anomaly_points_meat) {
  # Ensure the trend and seasonal components are not NA
  data_anomalized_meats$TOTAL_SALES_replaced[i] <- trend_component_meat[i] + seasonal_component_meat[i]
}
ts_meats_cleaned <- ts(data_anomalized_meats$TOTAL_SALES_replaced, frequency = 365, start = c(2013, 1))
autoplot(ts_meats_cleaned)
...
/////multivariate_ts_diffed <- diff(multivariate_ts, 1)
...
# Function to normalize and store values
normalize_and_store <- function(ts_data) {
  result <- normalize_min_max(ts_data)
  list(
    scaled = result$scaled,
...
# Normalize the time series and store the values
combined_ts_normalized_obj <- normalize_and_store(multivariate_ts_diffed)
combined_ts_normalized <- combined_ts_normalized_obj$scaled


#INVERT BACK
res <- invert_min_max_scaling(combined_ts_normalized,combined_ts_normalized_obj$min, combined_ts_normalized_obj$max)




# fitting model with AR order 4,0,3 
Model5 <- define.model(kvar=5, ar=c(4,1,3), ma=1)
Marima5 <- marima( combined_ts_normalized, Model5$ar.pattern, Model5$ma.pattern,
                   penalty=1)


nstart  <- 1668,
nstep   <- 15
Forecasts <- arma.forecast(series=combined_ts_normalized, marima=Marima5,
                           nstart=nstart, nstep=nstep )

Resid <- arma.filter( combined_ts_normalized, Marima5$ar.estimates,
                      Marima5$ma.estimates)

plot(Marima5$residuals[2, 5:1000], Resid$residuals[2, 5:1000],
